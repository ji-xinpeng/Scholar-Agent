...
![image-20260218131109138](assets/image-20260218131109138.png)

### 2.2 架构图详细说明

系统架构分为四层：接入层、应用层、工具层、基建层。
...
| 模块 | 职责 | 说明 |
|------|------|------|
| **任务规划** | 将用户复杂目标拆分为可执行 TODO 列表 | ✅ Agent Service |
| **工具调用** | 协调调用各工具完成任务 | ✅ Agent Service |
| **会话管理** | 上下文压缩、会话 CRUD | ✅ SessionService |
| **RAG 问答** | 基于检索的问答能力 | 📅 V1.0 |
...
```mermaid
flowchart TB
    subgraph title["Scholar Agent 智能学术助手"]
    end
    ...
    subgraph app["应用层<br/>Agent Application"]
        direction TB
        subgraph app_inner[" "]
            direction LR
            B1[任务规划]
    ...
    subgraph tools["工具层"]
        direction LR
        subgraph t_doc["Document工具"]
            direction TB
            T1[多模态解析]
            T2[文档编辑]
            T3[知识库构建]
        end
        subgraph t_search["检索工具"]
            direction TB
            T4[联网检索]
            T5[本地检索]
            T6[筛选排序]
        end
        subgraph t_chat["会话工具"]
            direction TB
            T7[会话CRUD]
            T8[上下文压缩]
            T9[记忆]
        end
        subgraph t_content["内容处理工具"]
            direction TB
            T10[归纳总结]
            T11[引用生成]
        end
        subgraph t_user["用户工具"]
            direction TB
            T12[用户画像]
            T13[个性化]
        end
    ...
    B1 & B2 & B3 & B4 & B7 & B8 --> T1
    B1 & B2 & B3 & B4 & B7 & B8 --> T4
    B1 & B2 & B3 & B4 & B7 & B8 --> T7
    B1 & B2 & B3 & B4 & B7 & B8 --> T10
    B1 & B2 & B3 & B4 & B7 & B8 --> T12
...
```
用户 → Web 前端 → RESTful API → Agent Application
                                 ↓
                          工具层（如文档编辑工具）
                                 ↓
                          文档管理 → SQLite / 文件系统
...
**命名与勘误说明**：
- Milvus：向量数据库正确拼写（非 Mvilus）
- 文档 Service：统一命名（非文件 Service）
...
sequenceDiagram
    ...
    loop SSE 事件流
        ...
        Agent-->>FE: event: doc_updated {doc_id} (可选
        FE->>FE: 实时更新消息 + 任务卡片 + 文档内容
    end
